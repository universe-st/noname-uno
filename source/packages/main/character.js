...
function readAvatarList(){
    return new Promise((resolve)=>{
        game.getFileList(basic.getExtensionRelativePath()+'resource/image/headimage',function(folders,files){
            resolve(files);
        });
    });
}
export const character = async function(){
    ...
    for(let i=0;i<avatars.length;i++){
        ...
        [`unoRandomPlayer`,'unseen',`des:<small>此名称“${name}”由代码在库中随机挑选字符拼凑生成，头像亦为随机选取，不代表作者看法。如有雷同或冒犯，纯属巧合，敬请谅解。</small>`,basic.getExtensionRelativePath().replace('extension/','ext:')+'resource/image/headimage/'+avatar]];
